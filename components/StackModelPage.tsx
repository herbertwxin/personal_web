  ...
  const models = [
    {
      ...
      content: {
        introduction: `Dynamic Stochastic General Equilibrium (DSGE) models represent the current frontier in macroeconomic modeling. This framework provides a rigorous foundation for policy analysis by grounding aggregate relationships in microeconomic optimization principles.`,

        sections: [
          {
            title: 'Model Setup and Environment',
            content: `We consider a small open economy populated by a representative household, firms, and a monetary authority. The economy is subject to both domestic and foreign shocks.

**Time Structure**: The model is set in discrete time t = 0, 1, 2, ...

**Information Structure**: All agents have rational expectations and observe the complete history of all variables up to time t.`,
          },

          {
            title: 'Household Problem',
            content: `The representative household maximizes expected lifetime utility:
...
where $\\sigma > 0$ is the coefficient of relative risk aversion, $\\nu > 0$ is the inverse of the Frisch elasticity of labor supply, and $\\chi > 0$ is a preference parameter.`,
          },

          {
            title: 'Budget Constraint and Optimality',
...
3. **Bond Demand:**
The Euler equation already incorporates the optimal bond choice.`,
          },

          {
            title: 'Firm Problem and Production',
...
$$(1-\\alpha) \\frac{Y_t}{L_t} = W_t$$`,
          },

          {
            title: 'Monetary Policy and Market Clearing',
            content: `**Monetary Policy Rule:**
...
where $\\delta$ is the depreciation rate.`,
          ...
          {
            title: 'Equilibrium and Solution Method',
            content: `**Definition of Equilibrium:**
...
where $x_t$ contains all endogenous variables and $\\varepsilon_t$ are the shocks.`,
          },
        ],

        codeExample: `// DSGE Model Solution - Simplified Calibration
const DSGEModel = {
...
console.log('IRF to 1% technology shock:', techShockIRF);`,

        mathematicalAppendix: `**A.1 Steady State Conditions**
...
4. Express in state-space form: $x_t = P z_t$`,
      },
    },

    {
      ...
      content: {
        introduction: `Endogenous growth theory revolutionized our understanding of long-term economic development by making technological progress an endogenous outcome of economic decisions. This toolkit provides comprehensive coverage of the mathematical foundations underlying modern growth theory.`,

        sections: [
          {
            title: 'The Basic AK Model',
            content: `We begin with the simplest endogenous growth model where capital is the sole input and there are no diminishing returns.
...
**Key Insight:** Growth is endogenous and depends on the savings rate $s$ and productivity $A$.`,
          },

          {
            title: 'Human Capital Accumulation',
            content: `We extend the basic model to include human capital as a separate factor.
...
**Effective Labor:**
$$L_t = l_t H_t = (1-u_t)H_t$$`,
          },

          {
            title: 'Optimization Problem',
            content: `**Representative Agent's Problem:**
...
**Hamiltonian:**
$$\\mathcal{H} = \\frac{C_t^{1-\\sigma}}{1-\\sigma} + \\lambda_t[AK_t^{\\alpha}(1-u_t)^{1-\\alpha}H_t^{1-\\alpha} - C_t - \\delta K_t] + \\mu_t[B u_t H_t - \\delta_H H_t]$$`,
          },

          {
            title: 'Optimality Conditions',
            content: `**First-Order Conditions:**
...
$$\\lim_{t \\to \\infty} e^{-\\rho t} \\mu_t H_t = 0$$`,
          },

          {
            title: 'Balanced Growth Path',
            content: `**Definition:** A balanced growth path (BGP) is a path where all variables grow at constant (possibly different) rates.
...
- Impatience ($\\rho$)  
- Risk aversion ($\\sigma$)
- Human capital depreciation ($\\delta_H$)`,
          },
        ],

        codeExample: `// Human Capital Growth Model - Numerical Solution
...
console.log('Transition dynamics computed');`,

        mathematicalAppendix: `**B.1 Existence and Uniqueness of BGP**
...
- $\\frac{\\partial g}{\\partial \\sigma} = -\\frac{B - \\delta_H - \\rho}{\\sigma^2} < 0$: Higher risk aversion reduces growth`,
      },
    },
  ]
    ...
    return (
      <motion.div className='min-h-screen pb-12 px-6 flex items-center justify-center'>
        <div className='text-center'>
          <h1 className='text-2xl text-black mb-4'>Model Not Found</h1>
          <Button
            onClick={onBack}
      ...
      <div className='max-w-5xl mx-auto'>
        ...
        >
          ...
          <div className='flex items-start justify-between mb-6'>
            <div>
              <h1 className='text-4xl text-black mb-4'>{model.title}</h1>
              ...
              <div className='flex items-center space-x-4 mb-4'>
                <Badge
                  variant='outline'
                  className={getDifficultyColor(model.difficulty)}
                >
                  {model.difficulty}
                </Badge>
                <span className='text-black text-sm'>{model.pages} pages</span>
        ...
        <motion.div
          ...
          className='mb-8 p-6 border border-[#e9e5ff] rounded-lg bg-white/50'
        >
          <div className='flex items-center mb-4'>
            <Lightbulb className='w-5 h-5 mr-2 text-[#6A5ACD]' />
            <h2 className='text-2xl text-black'>Introduction</h2>
          </div>
          <ProcessedText className='text-black leading-relaxed'>
        ...
        {model.content.sections.map((section, index) => (
          <motion.div
            ...
            className='mb-8 p-6 border border-[#e9e5ff] rounded-lg bg-white/50'
          >
            <h2 className='text-2xl text-black mb-4 flex items-center'>
              <Calculator className='w-5 h-5 mr-2 text-[#6A5ACD]' />
              {section.title}
            </h2>
            <ProcessedText className='text-black leading-relaxed whitespace-pre-line'>
              {section.content}
            </ProcessedText>
          </motion.div>
        ))}
        ...
        >
          <h2 className='text-2xl text-black mb-4 flex items-center'>
            <FileText className='w-5 h-5 mr-2 text-[#6A5ACD]' />
            Implementation Example
          </h2>
          <div className='bg-gray-900 text-green-400 p-6 rounded-lg font-mono text-sm overflow-x-auto'>
            <pre>{model.content.codeExample}</pre>
          </div>
        </motion.div>
        ...
        >
          <h2 className='text-2xl text-black mb-4 flex items-center'>
            <TrendingUp className='w-5 h-5 mr-2 text-[#6A5ACD]' />
            Mathematical Appendix
          </h2>
          <ProcessedText className='text-black leading-relaxed whitespace-pre-line'>
            {model.content.mathematicalAppendix}
          </ProcessedText>
        </motion.div>
        ...
        <motion.div
          ...
          className='flex justify-between items-center pt-8 border-t border-[#d6ceff]'
        >
          <Button
            onClick={onBack}
            variant='outline'
